...
Queue._rescheduleKeyBase = 'reschedule:job:';
...
Queue.prototype._rescheduleJob = function (job, done) {
    ...
    if (job.data._shedule && job.data._shedule.type == 'recurrent') {
        self.warlock.lock('lock:job:reschedule:'+jobId, lockTtl, function( err, unlock ) {
            if (err) {
                self.emit('error', err);
                return done(err);
            }
                ...
                _computeNextRunTime(clonedJob, function (result) {
                    if (result instanceof Error) {
                        ...
                        clonedJob.delay(result).save(function (err) {
                            ...
                            self._replaceRescheduleKey(jobId, clonedJob.id, function (err) {
                                unlock();
...
Queue.prototype._jobEventHandler = function (event, jobId, result) {
    ...
    Job.get(jobId, function (err, job) {
        if (err) {
            self.emit('error','error fetching job in jovEventHandler: '+err);
            return;
        }
...
Queue._rescheduleKey = function(jobId) {
    return Queue._rescheduleKeyBase + jobId;
};

Queue.prototype._createRescheduleKey = function (jobId, done) {
    done = done || noop;

    this.client.set(Queue._rescheduleKey(jobId), jobId, done);
};

Queue.prototype._removeRescheduleKey = function (jobId, done) {
    done = done || noop;

    this.client.del(Queue._rescheduleKey(jobId), done);
};

Queue.prototype._replaceRescheduleKey = function (oldJobId, newJobId, done) {
    done = done || noop;

    let multi = this.client.multi();

    multi
        .del(Queue._rescheduleKey(oldJobId))
        .set(Queue._rescheduleKey(newJobId),newJobId);
...
Queue.prototype._rescheduleJobs = function (done) {
    done = done || noop;

    let self = this;

    this.client.keys(Queue._rescheduleKeyBase + '*', function (err, keys) {
        if (err) return done(err);

        let jobIds = keys.map(key => key.split(':').pop());
        ...
        for (let jobId of jobIds) {
            promises.push(new Promise(function (fulfill, reject) {
                Job.get(jobId, function (err, job) {
                    if (err) {
                        return fulfill({ jobId: jobId, error: err });
                    }

                    if (job._state == 'complete' || job._state == 'failed') {
                        self._rescheduleJob(job, function (err) {
                            fulfill({ jobId: job.id, error: err || null });
                        });
                    } else {
                        fulfill({});
                    }
...
kue.createQueue = function(options) {
    ...
    if (!subscribed) {
        ...
        queue.on('job failed', function (jobId, error) {
            queue._jobEventHandler('failed', jobId, error);
        });
    }
...
var setupTimers = Queue.prototype.setupTimers;
Queue.prototype.setupTimers = function () {
    ...
    if (rescheduled == false) {
        ...
        console.log('[' + new Date + '] checking for jobs that needs to be rescheduled...');

        this._rescheduleJobs(function (err) {
...
Queue.prototype.every = function (recurrence, job, done) {
    ...
    _computeNextRunTime(job, function (result) {
        ...
        job.save(function (err) {
            ...
            self._createRescheduleKey(job.id, function (err) {
                if (err) {
                    self.emit('schedule error', err);
                    job.remove();
                }
