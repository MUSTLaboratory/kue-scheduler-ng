        ...
        self.warlock.lock(self.client.getKey('lock:job:reschedule:'+jobId), lockTtl, function( err, unlock ) {
            ...
            if (typeof unlock === 'function') {
                self.client.sismember(self.client.getKey('rescheduled:jobs'), jobId, function (err, result) {
                    ...
                    clonedJob.data._schedule.rescheduled = true;
...
Queue.prototype._jobEventHandler = function (event, jobId, result) {
    ...
    if (event == 'complete' || event == 'failed') {
        self.client.srem(self.client.getKey('rescheduled:jobs'), jobId, function (err) {
            if (err) {
                console.log('failed to remove rescheduled job from set', err);
            }
        });
        return;
    }
...
Job.prototype.isRescheduled = function () {
    return this.data._schedule.rescheduled == true;
};

var active = Job.prototype.active;
Job.prototype.active = function( clbk ) {
    var self = this;

    kue.singleton._rescheduleJob(this, function (err) {
        if (err) return self.inactive();

        active.call(self, clbk);
    });
};
//
//var inactive = Job.prototype.inactive;
//Job.prototype.inactive = function( clbk ) {
//    var self = this;
//
//    if (this._state == 'delayed') {
//        kue.singleton._rescheduleJob(this, function (err) {
//            inactive.call(self, clbk);
//        });
//    } else {
//        inactive.call(self, clbk);
//    }
