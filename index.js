...
function _computeNextRunTime(job, done) {
    let jobScheduleData = job.data.schedule;
...
function _jobEventHandler(event, jobId, result) {
    ...
    kue.Job.get(jobId, function (err, job) {
        ...
        if (event == 'complete' || event == 'failed') {
            if (job.data.schedule && job.data.schedule.type == 'recurrent') {
                let clonedJob = _cloneJob(self, job);
                clonedJob.data.schedule.lastRun = new Date(parseInt(job.started_at));
                //console.log('last run:',clonedJob.data.schedule.lastRun);
